...
// disableCommand creates a Cobra command for disabling the MCP server in Cursor.
func disableCommand() *cobra.Command {
	...
	// Add flags
	flags := cmd.Flags()
	flags.StringVar(&disableFlags.configPath, cfgmgr.FlagConfigPath, "", "Path to Cursor config file")
	...
	flags.BoolVar(&disableFlags.workspace, cfgmgr.FlagWorkspace, false, "Remove from workspace settings (.cursor/mcp.json) instead of user settings")
...
func disableMCPServer(flags *disableCommandFlags) error {
	...
	if err := configManager.RemoveServer(serverName); err != nil {
		return fmt.Errorf("failed to remove MCP server %q from Cursor configuration: %w", serverName, err)
	}

	fmt.Printf(cfgmgr.MsgServerDisabled, serverName)
	fmt.Printf("Configuration: %s\n", configType)
	fmt.Printf("Configuration file: %s\n", configManager.GetConfigPath())
	fmt.Print("\nTo apply changes, restart Cursor or reload the window.\n")
	return nil
}
