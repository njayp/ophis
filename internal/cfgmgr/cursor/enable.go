...
// enableCommand creates a Cobra command for enabling the MCP server in Cursor.
func enableCommand() *cobra.Command {
	...
	flags.StringVar(&enableFlags.configPath, cfgmgr.FlagConfigPath, "", "Path to Cursor config file")
	...
	flags.BoolVar(&enableFlags.workspace, cfgmgr.FlagWorkspace, false, "Add to workspace settings (.cursor/mcp.json) instead of user settings")
...
func enableMCPServer(cmd *cobra.Command, flags *enableCommandFlags) error {
	...
	if err != nil {
		return fmt.Errorf("failed to check if MCP server %q exists in Cursor configuration: %w", serverName, err)
	}
	...
	if err := configManager.AddServer(serverName, server); err != nil {
		return fmt.Errorf("failed to add MCP server %q to Cursor configuration: %w", serverName, err)
	}
	...
	fmt.Printf("\nTo use this server:\n")
	fmt.Printf("1. Open Cursor AI Chat\n")
	fmt.Printf("2. Use @ to access MCP tools\n")
	return nil
}
